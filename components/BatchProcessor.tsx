...
const BatchProcessor: React.FC<BatchProcessorProps> = ({ 
  ...
  const exportToExcel = () => {
    ...
    if (currentMarketplace === Marketplace.AJIO) {
      const headers = ['AJIO SKU*', 'ARTICLE CODE*', 'ASIN*', 'AMAZON SKU*', 'TP COST*', ...bufferCols, 'FINAL TP COST', 'AVG MRP', 'Trade Discount %', 'SALE DISCOUNT AMT', 'ASP (GROSS)', 'GST % on ASP', 'GST Amt on ASP', 'Net Sales Value', 'AJIO Margin %', 'AJIO Margin (Rs.)', 'Purchase price', 'GST % (Purchase)', 'GST (Purchase)', 'BANK SETTLEMENT', 'NET PROFIT', 'ROI %'];
      wsData.push(headers);
      results.forEach(item => {
        const profit = round(item.totalActualSettlement - (item.baseTp || 0));
        const roi = (item.baseTp && item.baseTp > 0) ? round((profit / item.baseTp) * 100) : 0;
      ...
      results.forEach(item => {
        ...
        if (isBuffersEnabled) row.push(round(item.baseTp * buffers.adsPercent / 100), round(item.baseTp * buffers.dealDiscountPercent / 100), round(item.baseTp * buffers.reviewPercent / 100), round(item.baseTp * buffers.profitMarginPercent / 100), round(item.baseTp * buffers.returnPercent / 100));
        row.push(round(item.targetSettlement), round(item.aisp), round(item.logisticsFee), round(item.customerPrice), `${item.commissionRate}%`, round(item.commission), round(item.fixedFee), item.level, round(item.fixedFee * GST_RATE), round(item.reverseLogisticsFee || 0), round((item.reverseLogisticsFee || 0) * GST_RATE), round(item.tcs || 0), round(item.tds || 0), round(item.totalActualSettlement), profit, `${roi}%`);
        wsData.push(row);
      });
    }
