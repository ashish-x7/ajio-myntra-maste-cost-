      ...
      <div className="bg-white p-4 rounded-2xl border border-forest-accent shadow-lg ring-1 ring-inset ring-white/60 space-y-4 dark:bg-forest-pine/40 dark:border-forest-leaf/30 overflow-hidden relative">
        ...
        <div className="flex items-center justify-between border-b border-forest-accent pb-3 dark:border-forest-leaf/20">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-black text-forest-pine uppercase tracking-tight dark:text-forest-mint">AJIO Analysis</h3>
          </div>
          <div className="px-2 py-0.5 bg-forest-pine text-white text-[7px] font-black uppercase rounded-full dark:bg-forest-leaf">Live</div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="mt-3 bg-forest-pine p-4 rounded-xl text-white flex justify-between items-center shadow-md relative overflow-hidden dark:bg-forest-leaf/40">
             <div className="relative z-10 flex flex-col">
                <span className="text-[7px] font-black text-forest-accent/60 uppercase tracking-widest mb-0.5">Bank Settlement</span>
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <div className="bg-forest-mint p-3 rounded-xl border border-forest-accent flex flex-col items-center text-center dark:bg-forest-pine/60 dark:border-forest-leaf/30 shadow-sm">
               <span className="text-[6px] font-black text-forest-leaf/50 uppercase tracking-widest mb-0.5">Net Profit</span>
               <span className={`text-base font-black ${netProfit >= 0 ? 'text-forest-leaf dark:text-forest-sage' : 'text-rose-600'}`}>
                 {format(netProfit)}
               </span>
            </div>
            <div className="bg-forest-mint p-3 rounded-xl border border-forest-accent flex flex-col items-center text-center dark:bg-forest-pine/60 dark:border-forest-leaf/30 shadow-sm">
               <span className="text-[6px] font-black text-forest-leaf/50 uppercase tracking-widest mb-0.5">ROI</span>
               <span className={`text-base font-black ${netProfit >= 0 ? 'text-forest-leaf dark:text-forest-sage' : 'text-rose-600'}`}>
                 {roi.toFixed(2)}%
               </span>
            </div>
    ...
    <div className="space-y-4 animate-in fade-in duration-700">
        ...
        <div className="relative z-10 space-y-4 text-center">
          ...
          <div className="flex justify-center gap-8 pt-4 border-t border-white/10">
            <div className="flex flex-col items-center">
              <span className="text-forest-accent/30 uppercase text-[6px] font-black tracking-widest mb-0.5">Consumer Price</span>
              <span className="text-xl font-black tracking-tight">{format(result.customerPrice)}</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-forest-accent/30 uppercase text-[6px] font-black tracking-widest mb-0.5">Logistics Cost</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-forest-accent shadow-md text-center flex flex-col justify-center items-center dark:bg-forest-pine/40 dark:border-forest-leaf/30 transition-all hover:scale-[1.01]">
          ...
          <span className={`text-2xl font-black tracking-tighter ${netProfit >= 0 ? 'text-forest-pine dark:text-forest-mint' : 'text-rose-600'}`}>
            {format(netProfit)}
          </span>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-forest-accent shadow-md text-center flex flex-col justify-center items-center dark:bg-forest-pine/40 dark:border-forest-leaf/30 transition-all hover:scale-[1.01]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-forest-accent shadow-sm space-y-3 dark:bg-forest-pine/40 dark:border-forest-leaf/30">
          <h4 className="text-[8px] font-black text-forest-leaf uppercase tracking-widest flex items-center gap-2 dark:text-forest-sage">
            <span className="w-1 h-1 bg-rose-500 rounded-full"></span> Platform Deductions
          </h4>
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <span className="text-[8px] font-bold text-forest-pine/40 uppercase">Commission %</span>
              <span className="font-black text-xs text-forest-pine dark:text-forest-mint">{result.commissionRate}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[8px] font-bold text-forest-pine/40 uppercase">Commission Amt</span>
              <span className="font-black text-xs text-forest-pine dark:text-forest-mint">{format(result.commission)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[8px] font-bold text-forest-pine/40 uppercase">Fixed Fee & GST</span>
              <span className="font-black text-xs text-forest-pine dark:text-forest-mint">{format(result.fixedFee * (1 + GST_RATE))}</span>
            </div>
            {result.reverseLogisticsFee > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-[8px] font-bold text-forest-pine/40 uppercase">Reverse Log. & GST</span>
                <span className="font-black text-xs text-forest-pine dark:text-forest-mint">{format(result.reverseLogisticsFee * (1 + GST_RATE))}</span>
              </div>
            )}
            <div className="pt-2 border-t border-forest-accent flex justify-between items-center dark:border-forest-leaf/20">
              <span className="text-[8px] font-black text-forest-pine uppercase dark:text-forest-mint">Total Leakage</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-forest-accent shadow-sm space-y-3 dark:bg-forest-pine/40 dark:border-forest-leaf/30">
          ...
          <div className="space-y-1.5">
            ...
            <div className="pt-2 border-t border-forest-accent flex justify-between items-center dark:border-forest-leaf/20">
              <span className="text-[8px] font-black text-forest-pine uppercase dark:text-forest-mint">Net Goal</span>
      ...
      <div className={`bg-white border rounded-2xl p-5 flex justify-between items-center shadow-lg dark:bg-forest-pine/60 relative overflow-hidden group/final transition-all duration-500 ${isMatched ? 'border-forest-leaf dark:border-forest-sage ring-2 ring-forest-leaf/5 shadow-[0_0_20px_rgba(45,90,58,0.05)]' : 'border-forest-accent'}`}>
        ...
        <div className="flex flex-col relative z-10">
          ...
          <span className="text-3xl font-black text-forest-pine tracking-tighter italic dark:text-forest-mint transition-all">
...
const AjioRow: React.FC<{ label: string; value: string; isBold?: boolean }> = ({ label, value, isBold }) => (
  <div className={`flex justify-between items-center px-3 py-1.5 rounded-lg transition-all ${isBold ? 'bg-forest-accent/20 dark:bg-forest-leaf/10' : 'hover:bg-forest-mint dark:hover:bg-forest-pine/60'}`}>
    <span className={`text-[7px] uppercase font-black tracking-widest ${isBold ? 'text-forest-pine dark:text-forest-mint' : 'text-forest-pine/40 dark:text-forest-sage/40'}`}>{label}</span>
    <span className={`text-xs font-black tracking-tight text-forest-pine dark:text-forest-mint`}>{value}</span>
  </div>
);
